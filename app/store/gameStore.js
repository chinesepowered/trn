...
const useGameStore = create(
  devtools(
    (set, get) => ({
      ...
      upgradeFighter: async (fighterId) => {
        const { user } = get();
        if (!user) return;
...
function generateRandomFighter() {
  ...
  return {
    ...
    name: generateFighterName(fighterClass),
    class: fighterClass,
    rarity: rarity,
    level: 1,
    experience: 0,
...
function generateFighterName(fighterClass) {
  const names = {
  ...
  const classNames = names[fighterClass];
  return classNames[Math.floor(Math.random() * classNames.length)];
}

async function simulateBattle(fighter1, fighter2) {
  ...
  return {
    ...
    rounds,
    rewards: [
      { type: 'ROOT_TOKENS', amount: winner === fighter1 ? 50 : 10 },
